...
```typescript
// React.memo - use for expensive renders
...
const handler = useCallback(() => {
  doSomething(counter); // ❌ If counter changes often
}, [counter]);

// DON'T memoize cheap calculations
const simple = useMemo(() => a + b, [a, b]); // ❌ Addition is cheap
```
